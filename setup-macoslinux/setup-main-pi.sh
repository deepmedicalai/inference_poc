...
# E.g. on a Mac:
#   ./setup-pi.sh /Volumes/boot
# or on Ubuntu:
#   ./setup-pi.sh /media/$USER/boot
...
then
  ...
  sed -i'.bak' -e "s/rootwait/rootwait modules-load=dwc2,g_ether/" "$CMDLINE_TXT_PATH"
else
  #echo "$CMDLINE_TXT_PATH already updated with modules and removed initial resize script."
  echo "$CMDLINE_TXT_PATH already updated with modules"
fi
